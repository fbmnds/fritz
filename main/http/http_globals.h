...
http_server_label_t set_payload_idx2 (str_pt* str, const char* recv_buf)
{
	str->len = 0;
    str->str = (char*)recv_buf + sizeof(char)*HTTP_RECV_BUF_LEN;
...
http_server_label_t set_payload_idx3 (str_pt* str, const char* recv_buf)
{
    str->len = 0;
    str->str = (char*)recv_buf + sizeof(char)*HTTP_RECV_BUF_LEN;
    ...
    //printf("set_payload_idx3: recv_buf\n'%s'\n", recv_buf);
    while (str->str != recv_buf) {
        //printf("%s", str->str);
        if ((str->str[0] >= '0' && str->str[0] <= '9') || 
